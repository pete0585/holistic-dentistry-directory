...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many holistic dentist are in Fort Worth, TX?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Holistic Dentist Directory lists 32+ holistic dentist in Fort Worth, Texas. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find holistic dentist in Fort Worth?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search holisticdentalfinder.com and filter by Fort Worth. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">Fort Worth, TX</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Holistic Dentist in Fort Worth, TX
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          32+ listed holistic dentist in the Fort Worth area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "Fort Worth")}, {String(row.state || "TX")}
              </p>
            </li>
          ))}
        </ul>
